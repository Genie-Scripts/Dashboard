...
def render_kpi_cards_inline(kpi: dict) -> str:
    ...
    cards = [
        inp_card,
        # ★F3: admission_gap は営業日期待値で補正済みのため、併記する目標値も
        # 補正後(admission_target_weekly_adj)に揃える（実績+目標差=目標 の整合を保つ）。
        single("admission", "新入院患者数", "直近7日累計", kpi["admission_actual_7d"], "人",
               kpi["admission_gap"], "人", kpi["admission_status"],
               kpi.get("admission_target_weekly_adj", TARGET_ADMISSION_WEEKLY),
