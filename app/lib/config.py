...
# 救命救急センター系病棟（緊急入院・院内転棟が中心で、他病棟のような「予定入院の曜日調整」
# 「地域医療連携（紹介元）への働きかけ」という業務前提が成り立たない）。
# 部門レポートPDFの「この期間の一手」は、この2病棟だけ専用の文言（dept_report._is_emergency_ward
# 経由）にする。ICU(04B)/HCU(04D)は前年比較除外(NO_PREVYEAR_WARDS)のみで、一手は通常通り。
