...
# 07B=オーバーナイト救急病棟（緊急で受けて短期転棟させる入口病棟。2026-08-29 ユーザー裁定で昇格）
EMERGENCY_WARDS = {"04A", "04C", "07B"}
...
# 病棟名 → 病棟コードの逆引き（WARD_NAMES は一意なので逆写像が作れる）
_WARD_CODE_BY_NAME = {n: c for c, n in WARD_NAMES.items()}


def unit_narration_kind(entity: str, code=None, name=None):
    """ナレーションの業務前提が通常と異なる特例ユニットの種別を返す（None=通常）。

      "emergency"     = 救命救急センター系病棟(4A/4C)
      "critical_care" = 重症ケア病棟(ICU=04B / HCU=04D)
      "er_dept"       = 救急科

    dept_report（部門レポートPDF・dept.html の一手）/ triage（ポータル部門トリアージ）/
    eval_rules（AIアラートのレバー注入）が同じ判定を共有するための単一の真実。
    病棟はコード・名称のどちらで渡されても解決する（呼び出し側のコード解決が
    失敗しても特例判定が落ちないようにするため＝ICU が一般病棟扱いされる回帰の防止）。
    """
    if entity == "ward":
        c = code if code in WARD_NAMES else _WARD_CODE_BY_NAME.get(name or code)
        if c in EMERGENCY_WARDS:
            return "emergency"
        if c in CRITICAL_CARE_WARDS:
            return "critical_care"
        return None
    if entity == "dept" and name in ER_DEPTS:
        return "er_dept"
    return None
...
# ユニットの役割別「打ち手（レバー）」
# ──────────────────────────────
# 病棟は外来も地域連携の窓口も持たないため、「紹介元への働きかけ」「地域医療連携の強化」
# 「予定入院枠の前倒し」は病棟の業務として成立しない（いずれも診療科のレバー）。
# 病棟が動かせるのは病床管理と受け入れ（緊急入院・転入）、退院/転棟のタイミング調整のみ。
# LLM プロンプトの【打ち手（レバー）】と、oMLX 未起動時の定型文の両方がここを参照し、
# 文言の乖離を防ぐ。キーは unit_narration_kind の戻り値（"ward"=特例でない一般病棟）。
UNIT_ROLE_LEVERS = {
    "ward": [
        "空床の把握とベッドコントロール（病床管理）で受け入れ可能な病床を確保する",
        "緊急入院・他病棟からの転入の受け入れを増やす",
        "退院・転棟のタイミングを調整し、週末・週明けの受け入れ余地をつくる",
        "担当診療科・病床管理部門と連携し、空床への入院調整を進める",
    ],
    "emergency": [
        "救急・緊急入院の受け入れを絶やさない",
        "転棟・転出（下り搬送）の判断を迅速化し、受け入れ余地を確保する",
        "ICU・HCU と連携して転入・転出のタイミングを調整する",
        "週末も平日と同水準の受け入れ体制（病床運用・当直）を保つ",
    ],
    "critical_care": [
        "院内急変・緊急術後の重症患者の受け入れを絶やさない",
        "手術部・救急・一般病棟と連携して重症患者の受け入れを円滑にする",
        "後方病床（一般病棟）への転棟タイミングを適正化し、高稼働と受け入れ余地を両立する",
        "週末も重症管理の運用・当直体制を平日と同水準に保つ",
    ],
    "er_dept": [
        "救急車の応需台数を増やす",
        "救急受入体制（当直・病床確保）を維持・強化する",
        "後方病床との連携で救急外来（ER）滞在時間を短縮する",
    ],
}

# 病棟・特例ユニットのナレーションに出てはいけない語（診療科専用のレバー）。
# LLM は一般知識から「紹介元への働きかけ」を補いやすいため、プロンプト指示だけでなく
# 生成後の機械ガード（禁止語）でも弾く多重防衛にする。
WARD_BANNED_LEVER_TERMS = ("紹介", "地域医療連携", "紹介元")
...
#
# 係数の出所: 2026-09-14 再測定（post=6/7/8月の中央値）。旧値 外来1.053/入院1.144 は
#   6月単月ベース。9月確報で再測定すること。
#   改定後確定月 = 2026-06〜08 の3か月分 leakage-free バックキャスト
#   （改定前 2026-03〜05 の actual/proj 比 median で正規化）。
#   旧係数(2026-07-17測定)は2026-06単月のみに基づき、同月は在院日数が3か月中最短で
#   推定法のバイアスが最大だった（他月を混ぜた今回の再測定でバイアスが緩和）。
...
# ──────────────────────────────
# 号令を「粗利/人日を上げろ」「在院日数を短くしろ」ではなく「在院は目標を守る＋
# 新入院あと○人/日＋期間III超え患者数を減らす」に置き換えるための設定。
# 恒等式: 在院患者数 ≒ 新入院/日 × 在院日数。在院目標を維持したまま新入院を
# 増やせば在院日数は結果として縮む＝在院日数の短縮そのものは号令にしない
# （延伸禁止に加え、短縮も号令にしない）。
TURN_REF_MONTHS = 12               # 基準在院日数＝直近N完全月の月次alos_proxy中央値
TURN_LOS_FEED_FOLDER = "los_data"  # 期間III超え患者数フィード（任意）
TURN_FILL_RATIO = 0.98             # 在院がこの比率×目標未満なら「まず床を埋める」
TURN_GAP_MIN = 0.5                 # 新入院のあと○人/日がこれ以上なら「回転」

# ルール文の語彙を1か所に固定（metrics.turnover_metrics の state → 文言）。
TURN_HINTS = {
    "fill": "まず床を埋める：在院あと{census_gap}人",
    "turn": "空いた床に次を入れる：新入院あと{nadm_gap}人/日。入院時に退院日を決める",
    "hold": "回転は維持。期間III超えを減らす（退院日未確定を毎朝確認）",
}
...
DATA_FOLDERS = {
    ...
    # 回転3指標③（期間III超え患者数）。任意フィード。未配置なら在院日数近似で代替表示する。
    "los_data": TURN_LOS_FEED_FOLDER,
}
