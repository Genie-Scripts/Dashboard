...
def rebuild_corpus(state_dir) -> int:
    ...
    try:
        ...
        # 最新の1件だけ残す（同着はファイル後方＝後勝ち）。反転後（§6-1: overrideは
        # base一致のときだけ毎ビルド自動適用される）は同じ添削文が複数base_dateへ
        # 自動で再捕捉される根本原因が解消済み。本dedupは保険として残す（手編集で
        # 明示的に「前回の添削を使う」を繰り返し押した場合の水増し対策）。
        # human_action が空文字の行は判定不能として重複排除の対象外とする
        # （空文字同士を同一視して間引かない）。
        dedup: dict = {}
        empties = []
        for row in rows:
