...
def _build_improvement(adm, surg, base_date) -> dict:
    """改善トピック: 北極星KPIの前週比で各群上位3件（プラスのみ）
       ...
       病棟   → 在院の前週同曜日比（人）
    build_portal_context() から抽出（B3週次ダイジェストからも再利用するため）。
    """
    def _wow_top3(cands):
    ...
    return {"dept_internal": dept_imp_internal,
    ...
    data = {
        ...
        "kpi": {
            ...
            "operation": {
                ...
                "range_7d": kpi["operation_7d_range"],
                "prev_range_7d": kpi["operation_7d_prev_range"],
                "last_week_total": kpi["operation_last_week_total"],
                "last_week_range": kpi["operation_last_week_range"],
