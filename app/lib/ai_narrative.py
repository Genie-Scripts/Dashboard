...
3. 在院日数の延長（退院を月曜まで遅らせる＝月曜延伸）や早期退院の促進は提案しない（禁止）。狙いはベッド回転であって延伸ではない（延伸も短縮も号令にしない）。
...
# 「延伸」「早期退院」= 回転3指標（在院日数の短縮そのものは号令にしない）と同じ制約を
# 新入院トピックでも機械的に弾く（leveling/hospital_summary の既存banned語と合わせる）。
...
2. ねらいは新入院（週間の入院受け入れ人数）を目標水準へ近づけること。具体策は「地域医療連携（紹介元）への働きかけ強化」「予定入院枠の繰り上げ・前倒しの検討」など、運用面の一般的な対応にとどめる。目標未達のときの action は「新入院の患者数増に取り組みましょう」のような直接的な呼びかけで締める。
...
4. 在院日数の延長や早期退院の促進は提案しない（禁止。延伸も短縮も号令にしない）。
5. 出力は指定 JSON のみ。前置き・説明・``` を付けない。簡潔・丁寧・事務的なトーン。
