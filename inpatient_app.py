...
<head>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .tab-btn.active { background: var(--accent); color: #fff; border-color: var(--accent); box-shadow: 0 1px 4px rgba(13,110,138,0.25); }
    ...
    .perf-row {
      display: grid;
      grid-template-columns: 120px 170px 1fr 52px 16px;
...
<header class="site-header">
  <div class="header-left">
    <span class="header-badge">在院</span>
    <span class="header-title">🏥 在院患者数ダッシュボード</span>
...
<div class="wrap">
  ...
  <div class="section-head">
    <span class="section-label">病院全体 在院患者 KPI</span>
  </div>
  ...
  <div class="chart-card" id="chart-container">
    ...
    <div class="chart-hint">💡 ホバーで詳細表示（縦軸: 日平均在院患者数）</div>
  </div>

  <!-- パフォーマンスバー -->
  <div class="section-head">
    <span class="section-label">部門別 在院患者 達成状況</span>
  </div>
  ...
  <div class="perf-legend">
    <div class="legend-item"><div class="legend-dot" style="background:var(--accent)"></div>縦線 = 目標ライン（日平均）</div>
    <div class="legend-item"><div class="legend-dot" style="background:var(--success)"></div>達成率 ≥ 100%</div>
    <div class="legend-item"><div class="legend-dot" style="background:var(--warning)"></div>90〜99%</div>
    <div class="legend-item"><div class="legend-dot" style="background:var(--danger)"></div>&lt; 90%</div>
  </div>
...
function renderKPI() {
  ...
  // 在院患者数の達成判定（目標以上が「達成」: 病床稼働率の観点）
  function clsCls(val, tgt) {
    return val >= tgt ? 'success' : val >= tgt * 0.9 ? 'warn' : 'danger';
  }
  ...
  const fyCls  = clsCls(k.fy_avg,   k.target_daily);
  const d7Cls  = clsCls(k.avg_7d,   k.target_daily);
  ...
  grid.innerHTML = [
    card(k.fy_year + '（日平均）',
         ...
         '目標 ' + k.target_daily + ' 人/日',
         fyCls, fyIcon,
         k.fy_avg / k.target_daily * 100,
         k.fy_avg.toFixed(1) + ' / ' + k.target_daily + ' 人/日'),
    card(k.fy_year + ' 達成率',
         k.fy_rate.toFixed(1) + '%',
         k.fy_avg.toFixed(1) + ' / ' + k.target_daily + ' 人/日',
         rateCls, rateIcon,
         ...
         k.fy_rate.toFixed(1) + '%'),
    card('直近7日 日平均',
         k.avg_7d.toFixed(1) + ' 人',
         '目標 ' + k.target_daily + ' 人/日',
         d7Cls, d7Icon,
         k.avg_7d / k.target_daily * 100,
         (k.avg_7d / k.target_daily * 100).toFixed(1) + '%'),
    card('直近7日 達成率',
         k.rate_7d.toFixed(1) + '%',
         '直近7日合計 ' + k.total_7d + ' 人',
         r7Cls, r7Icon,
         k.rate_7d,
         k.rate_7d.toFixed(1) + '%'),
    card((k.prev_year || '昨年度') + '（日平均）',
         k.prev_avg !== null ? k.prev_avg.toFixed(1) + ' 人' : '―',
         k.prev_avg !== null ? '前年比 ' + prevRatio : 'データ未入力',
         'flat', '―',
         k.prev_avg !== null ? k.prev_avg / k.target_daily * 100 : null,
         k.prev_avg !== null ? (k.prev_avg / k.target_daily * 100).toFixed(1) + '%' : ''),
  ].join('');
}
...
function getChartConfig(view, key, rangeKey) {
  ...
  const accentC  = '#0D6E8A';
  const accentBg = 'rgba(13,110,138,0.08)';

  // ── 病院/病棟全体 ──
  if (key === '__all__') {
    const cd   = sliceByRange(DATA.chart, fromStr);
    const tgt  = DATA.kpi.target_daily;
    const prev = DATA.kpi.prev_avg !== null ? DATA.kpi.prev_avg : null;
    ...
    const datasets = [
      ...
      { label: '28日移動平均', data: cd.map(d => d.ma),
        ...
        borderDash: [5, 3] },
      { label: '目標 ' + DATA.kpi.target_daily + '人/日',
    ...
    if (prev !== null) datasets.push({
      label: DATA.kpi.prev_year + ' ' + DATA.kpi.prev_avg.toFixed(1) + '人/日',
  ...
  if (view === 'dept') {
    ...
    if (prev !== null) datasets.push({
      label: DATA.kpi.prev_year + ' ' + prev.toFixed(1) + '人/日',
      data: Array(cd.length).fill(prev),
      borderColor: '#1A9E6A', borderDash: [4,4], borderWidth: 1.5, pointRadius: 0, fill: false
    });
    return { labels, datasets, yMin, yMax };
...
function buildPerfRows(rows) {
  ...
  return rows.map(r => {
    const cls     = r.rate >= 100 ? 'achieved' : (r.rate >= 90 ? 'near' : 'below');
    ...
    return `<div class="perf-row">
      ...
      <div class="perf-nums">${r.actual}人/日 / 目標${r.target}人</div>
...
def load_and_process_from_dir(data_dir: str, prev_avg_daily=None):
    """
    data/patient_data/ と data/patient_target/ から直接読み込んで集計する。
    generate_html.py の make deploy 統合用。
    prev_avg_daily: 昨年度の日平均在院患者数（人/日）。Noneの場合はデータから計算。
    """
    import sys
    ...
    return _process_inpatient_df(df, df_target, prev_avg_daily)


def load_and_process(log_files, target_file, prev_avg_daily=None):
    """
    Streamlit版: アップロードされたファイルオブジェクトから読み込んで集計する。
    prev_avg_daily: 昨年度の日平均在院患者数（人/日）。Noneの場合は「データなし」表示。
    """
...
def _process_inpatient_df(df: pd.DataFrame, df_target: pd.DataFrame, prev_avg_daily=None):
    """
    前処理済み DataFrame + 目標 DataFrame から KPI / チャート / パフォーマンスを集計。
    在院患者数の目標は「日平均（人/日）」単位。
    """
    df = df[df['診療科名'].notna()].copy()
    if '在院患者数' not in df.columns:
        raise ValueError("データに「在院患者数」列がありません。")
    df['在院患者数'] = pd.to_numeric(df['在院患者数'], errors='coerce').fillna(0).astype(int)

    # ── 目標設定済み診療科を特定（「日平均在院患者数」「診療科」「全日」行）
    tgt_dept_all = df_target[
        (df_target['部門種別'] == '診療科') &
        (df_target['指標タイプ'] == '日平均在院患者数') &
        (df_target['期間区分'] == '全日') &
        (~df_target['部門名'].isin(['病院全体', '内科']))
    ].copy()

    target_depts = set(tgt_dept_all['部門名'].tolist())
    ...
    if prev_fy_from_actual <= prev_fy_to_actual:
        ...
        # 在院患者数は「各日の病院全体合計の平均」
        prev_daily_by_date = df[prev_mask].groupby('日付')['在院患者数'].sum()
        prev_avg_data = float(prev_daily_by_date.mean()) if len(prev_daily_by_date) > 0 else None
    else:
        prev_avg_data = None

    # 引数で渡された値を優先、なければデータから計算
    if prev_avg_daily is not None:
        prev_avg = prev_avg_daily
    elif prev_avg_data is not None:
        prev_avg = prev_avg_data
    else:
        prev_avg = None

    # ── 病院全体目標（日平均在院患者数・病棟・全日）
    row_hosp = df_target[
        (df_target['部門名']   == '病院全体') &
        (df_target['部門種別'] == '病棟') &
        (df_target['指標タイプ'] == '日平均在院患者数') &
    ...
    target_daily = float(row_hosp['目標値'].values[0]) if len(row_hosp) else 567.0

    # ── KPI（今年度ベース）
    fy_mask = (df['日付'] >= fy_from) & (df['日付'] <= date_max)
    # 日別合計の平均
    fy_daily_by_date = df[fy_mask].groupby('日付')['在院患者数'].sum()
    fy_avg  = float(fy_daily_by_date.mean()) if len(fy_daily_by_date) > 0 else 0.0
    fy_rate = fy_avg / target_daily * 100

    # 直近7日
    d7_mask = df['日付'] >= d7_from
    d7_daily = df[d7_mask].groupby('日付')['在院患者数'].sum()
    avg_7d  = float(d7_daily.mean()) if len(d7_daily) > 0 else 0.0
    total_7d = int(d7_daily.sum())
    rate_7d = avg_7d / target_daily * 100

    kpi = {
        'target_daily': round(target_daily, 1),
    ...
    # ── 時系列（直近365日を上限、MA28は全期間から計算）
    all_dates_idx  = pd.date_range(date_min, date_max)
    # 日別病院全体在院患者数
    daily_all_full = df.groupby('日付')['在院患者数'].sum().reindex(all_dates_idx).fillna(0)
    ...
    def calc_perf(df_sub, group_col, df_tgt, from_date, to_date):
        """日平均在院患者数で集計（各日の合計の平均）"""
        mask = (df_sub['日付'] >= from_date) & (df_sub['日付'] <= to_date)
        df_period = df_sub[mask]
        # 各グループの日別平均
        grp_daily = df_period.groupby(['日付', group_col])['在院患者数'].sum().reset_index()
        actuals = grp_daily.groupby(group_col)['在院患者数'].mean()
        rows = []
    ...
    perf = {
        'dept': {
            '7':  calc_perf(df, '診療科名', tgt_dept, d7_from,  date_max),
            '28': calc_perf(df, '診療科名', tgt_dept, d28_from, date_max),
            'fy': calc_perf(df, '診療科名', tgt_dept, fy_from,  date_max),
        },
        'ward': {
            '7':  calc_perf(df, '病棟名', tgt_ward, d7_from,  date_max),
            '28': calc_perf(df, '病棟名', tgt_ward, d28_from, date_max),
            'fy': calc_perf(df, '病棟名', tgt_ward, fy_from,  date_max),
        }
    }
    ...
    for _, trow in tgt_dept.iterrows():
        dname = trow['部門名']
        daily_tgt = float(trow['目標値'])
        df_d  = df[df['診療科名'] == dname].copy()

        # 前年度日平均（診療科別）
        if prev_fy_from_actual <= prev_fy_to_actual:
            pm = (df_d['日付'] >= prev_fy_from_actual) & (df_d['日付'] <= prev_fy_to_actual)
            prev_d_daily = df_d[pm].groupby('日付')['在院患者数'].sum()
            prev_daily_d = round(float(prev_d_daily.mean()), 3) if len(prev_d_daily) > 0 else None
        else:
            prev_daily_d = None

        # 全期間MA28 → 直近365日分のみ格納
        all_d  = df_d.groupby('日付')['在院患者数'].sum().reindex(all_dates_idx).fillna(0)
        ...
        dept_targets[dname] = {'daily': daily_tgt, 'prev_daily': prev_daily_d}
    ...
    for _, trow in tgt_ward.iterrows():
        wname     = trow['部門名']
        daily_tgt_w = float(trow['目標値'])
        df_w      = df[df['病棟名'] == wname].copy()

        # 前年度日平均（病棟別）
        if prev_fy_from_actual <= prev_fy_to_actual:
            pm = (df_w['日付'] >= prev_fy_from_actual) & (df_w['日付'] <= prev_fy_to_actual)
            prev_w_daily = df_w[pm].groupby('日付')['在院患者数'].sum()
            prev_daily_w = round(float(prev_w_daily.mean()), 3) if len(prev_w_daily) > 0 else None
        else:
            prev_daily_w = None

        # 全期間MA28計算 → 直近365日分のみ格納
        all_w  = df_w.groupby('日付')['在院患者数'].sum().reindex(all_dates_idx).fillna(0)
        ...
        ward_targets[wname] = {'daily': daily_tgt_w, 'prev_daily': prev_daily_w}
...
def generate_html(kpi, chart_data, perf, meta, dept_chart, dept_targets, ward_chart, ward_targets):
    ...
    # 縦軸レンジ計算
    tgt_daily  = kpi['target_daily']
    prev_daily = kpi['prev_avg']
...
if __name__ == "__main__":
    ...
    st.set_page_config(page_title="在院患者ダッシュボード生成", layout="centered")

    st.title("🏥 在院患者数ダッシュボード 生成ツール")
    ...
    with st.sidebar:
        ...
        prev_input  = st.number_input(
            ...
            help="入力するとKPIに前年比が表示されます。0のまま生成するとデータから自動計算します。"
        )
        prev_avg_daily = prev_input if prev_input > 0 else None
    ...
    if log_files and target_file:
        if generate_btn:
            with st.spinner("集計・HTML生成中..."):
                try:
                    kpi, chart_data, perf, meta, dept_chart, dept_targets, ward_chart, ward_targets = load_and_process(log_files, target_file, prev_avg_daily)
    ...
    if 'html_ready' in st.session_state:
        ...
        # docsフォルダへ自動保存
        import os
        docs_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "docs", "inpatient")
        os.makedirs(docs_dir, exist_ok=True)
        save_path = os.path.join(docs_dir, "index.html")
        with open(save_path, "w", encoding="utf-8") as f:
            f.write(html_str)
        st.success(f"📁 `docs/inpatient/index.html` に保存しました（{save_path}）")

        # ダウンロードボタン
        st.download_button(
            label="⬇️ HTMLファイルをダウンロード",
            data=html_str.encode('utf-8'),
            file_name="inpatient_index.html",
            mime="text/html",
            type="primary"
        )
        ...
        c1.metric(kpi['fy_year'] + "（日平均）", f"{kpi['fy_avg']:.1f}人", f"目標 {kpi['target_daily']}/日")
        c2.metric(kpi['fy_year'] + " 達成率",    f"{kpi['fy_rate']:.1f}%")
        c3.metric("直近7日 日平均",              f"{kpi['avg_7d']:.1f}人")
        c4.metric("直近7日 達成率",              f"{kpi['rate_7d']:.1f}%")
        prev_disp = f"{kpi['prev_avg']:.1f}人" if kpi['prev_avg'] is not None else "未入力"
        c5.metric(kpi.get('prev_year','昨年度') + "（日平均）", prev_disp)
        ...
        with st.expander("📌 使い方"):
            st.markdown("""
            1. **実績データ**（`入退院.xlsx` など）をアップロード
            2. **目標値CSV**（`入院患者ダッシュボード_目標値.csv`）をアップロード
            3. 昨年度の日平均在院患者数を入力（任意・省略時はデータから計算）
            4. 「🚀 HTMLを生成する」をクリック
