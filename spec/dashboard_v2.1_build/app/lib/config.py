...
def build_headline(kpi_summary: dict) -> dict:
    ...
    failed = {k: v for k, v in rates.items() if v is not None and v < THRESHOLD_OK}
    n_fail = len(failed)

    if n_fail == 0:
        level, icon, text = "ok", "🟢", "全指標が目標を達成しています"
    elif n_fail == 1:
        worst = min(failed, key=failed.get)
        level, icon = "warn", "🟡"
        text = f"{worst}が目標をやや下回っています"
    elif n_fail == 2:
        worst = min(failed, key=failed.get)
        level, icon = "danger", "🔴"
        text = f"{worst}が目標を下回っています"
    else:
        worst = min(failed, key=failed.get)
        level, icon = "danger", "🔴"
        text = f"{worst}が目標を大きく下回っています"
